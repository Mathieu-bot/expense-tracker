...
const NotificationBell = ({hasNotifications,notifNumber} : {hasNotifications : boolean,notifNumber: number})  => {
  return (
    <button
      className="outlin-none border-none rounded-full p-2 bg-gray-800/20 backdrop-blur-2xl hover:bg-gray-400/30 transition-colors duration-200 shadow-md z-10 flex items-center justify-center cursor-pointer text-white"
    >
      <Bell size={24}/>
      {hasNotifications && (
